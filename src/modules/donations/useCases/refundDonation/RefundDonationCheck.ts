...
import { refundDonationSchema } from "./RefundDonationSchema";

async function checkBody(donationID: Donations["id"], chargeID:string): Promise<validationResponse> {
    // check body properties
    try {
        const yupValidation = await refundDonationSchema.validate({donationID, chargeID}, {
            abortEarly:false,
        })
